    ...
    def __init__(self, *, configs_dir: Optional[str] = None):
        ...
        if self.standalone:
            ...
            self.sidechain_bootstrap_config = ConfigFile(
                file_name=f"{self.configs_dir}/main.no_shards.dog.sidechain/sidechain_bootstrap.cfg"
            )
        else:
            self.mainchain_config = ConfigFile(
                file_name=f"{self.configs_dir}/sidechain_testnet/main.no_shards.mainchain_0/rippled.cfg"
            )
            self.sidechain_config = ConfigFile(
                file_name=f"{self.configs_dir}/sidechain_testnet/sidechain_0/rippled.cfg"
            )
            self.sidechain_bootstrap_config = ConfigFile(
                file_name=f"{self.configs_dir}/sidechain_testnet/sidechain_0/sidechain_bootstrap.cfg"
            )
        ...
        self.federators = [
            l.split()[1].strip()
            for l in self.sidechain_bootstrap_config.sidechain_federators.get_lines()
        ]

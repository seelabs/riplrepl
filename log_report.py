...
_has_256bit_hex_field_overloads = defaultdict(lambda: _has_256bit_hex_field_other)
...
def _rekey_dict_by_txn_date(hash_to_timestamp: dict, grouped_by_txn: dict) -> dict:
    """
    ...
    """
    known_txns = [k for k, v in sorted(hash_to_timestamp.items(), key=lambda x: x[1])]
    result = {}
...
def _to_timestamp(str_time: str) -> datetime.datetime:
    return datetime.datetime.strptime(str_time.split(".")[0], "%Y-%b-%d %H:%M:%S")
...
class Report:
    def __init__(self, in_dir, out_dir):
        ...
        self.counts_by_txn_and_server_file_name = (
            f"{self.out_dir}/counts_by_txn_and_server.org"
        )
        ...
        if os.path.exists(self.out_dir):
            if not os.path.isdir(self.out_dir):
                eprint(f"The output: {self.out_dir} exists and is not a directory")
                sys.exit(1)
        else:
        ...
        for txn_hash, server_dict in self.grouped_by_txn.items():
            ...
            df = pd.DataFrame(0, index=server_dict.keys(), columns=message_list)
            for server_id, messages in server_dict.items():
                for m in messages:
        ...
        self.grouped_by_txn = _rekey_dict_by_txn_date(
            hash_to_timestamp, self.grouped_by_txn
        )
        ...
        with open(self.counts_by_txn_and_server_file_name, "w") as out:
            for txn_hash, df in counts_by_txn_and_server.items():
                ...
                if txn_hash in hash_to_txn:
                    print(json.dumps(hash_to_txn[txn_hash], indent=1), file=out)
                rename_dict = {}
                for column, renamed_column in zip(
                    df.columns.array, string.ascii_uppercase
                ):
